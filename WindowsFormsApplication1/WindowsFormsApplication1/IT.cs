...
namespace WindowsFormsApplication1
{
    public partial class IT : Form
    {
        ...
        private void DisplayData()
        {
            ...
            adapt = new SqlDataAdapter("select * from ITStream", con);
            adapt.Fill(dt);
        ...
        {
            cmd = new SqlCommand("insert into ITStream(BatchID,name, stream, NoOfSubjects, subject1 , subject2, subject3, subject4, subject5)values(@BatchID,@name, @stream,@NoOfSubjects, @subject1 , @subject2, @subject3 ,@subject4, @subject5)", con);
            con.Open();
        ...
        {
            cmd = new SqlCommand("update ITStream set BatchID=@BatchID,name=@name, stream=@stream, NoOfSubjects=@NoOfSubjects, subject1=@subject1 , subject2=@subject2, subject3=@subject3, subject4=@subject4, subject5=@subject5  where ID=@ID", con);
            con.Open();
            cmd.Parameters.AddWithValue("@ID", ID);
            cmd.Parameters.AddWithValue("@BatchID", txtBatchID.Text);
            cmd.Parameters.AddWithValue("@name", txtname.Text);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (ID != 0)
            {
                cmd = new SqlCommand("delete ITStream where ID=@ID", con);
                con.Open();
                cmd.Parameters.AddWithValue("@ID", ID);
                cmd.ExecuteNonQuery();
                con.Close();
