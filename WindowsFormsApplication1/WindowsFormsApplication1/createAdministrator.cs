...
namespace WindowsFormsApplication1
{
    public partial class createAdministrator : Form
    {
        ...
        private void DisplayData()
        {
            ...
            adapt = new SqlDataAdapter("select * from create_administrator", con);
            adapt.Fill(dt);
        ...
        {
            cmd = new SqlCommand("insert into create_administrator(username,employeeID, password, gender, mobile, job, address)values(@username,@employeeID, @password,@gender, @mobile, @job ,@address)", con);
            con.Open();
        ...
        {
            cmd = new SqlCommand("update create_administrator set username=@username,employeeID=@employeeID, password=@password, gender=@gender, mobile=@mobile, job=@job, address=@address  where ID=@ID", con);
            con.Open();
            cmd.Parameters.AddWithValue("@ID", ID);
            cmd.Parameters.AddWithValue("@username", txtname.Text);
            cmd.Parameters.AddWithValue("@employeeID", txtid.Text);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (ID != 0)
            {
                cmd = new SqlCommand("delete create_administrator where ID=@ID", con);
                con.Open();
                cmd.Parameters.AddWithValue("@id", ID);
                cmd.ExecuteNonQuery();
                con.Close();
